...
/**
 * Interface de répository pour les opérations CRUD sur les objets Epci.
 *
 ...
 */
public interface EpciRepository extends GenericRepository<Epci,String> {


	/**
	 * Méthode pour trouver les objets Epci qui se trouvent dans une boîte englobante donnée.
	 *
	 ...
	 *
	 * @return une liste d'objets Epci qui se trouvent dans la boîte englobante donnée.
	 */
	 @Query(value= """
                SELECT e.id as id, e.nom_epci as nom, e.siren_epci as code, public.ST_REDUCEPRECISION(g.the_geom,:precision) as geomInt
                FROM ref_epci e
                INNER JOIN ref_epci_geometry g ON e.id=g.id
  ...
  @Query(value="""
                SELECT DISTINCT g.id,siren_epci as code,nom_epci as nom,
                ...
                cross join ref_epci d
                join ref_epci_geometry g ON d.id=g.id
                where
                ...
                """,nativeQuery=true)
  List<SelectionZonage> selectionInterieurExterieurEpciInRegion(@Param("regionId") List<Long> regionId, BigDecimal precision);
  ...
  @Query(value="""
                ...
                """,nativeQuery=true)
  List<SelectionZonage> selectionInterieurExterieurEpciInDepartement(@Param("departementId") List<Long> departementId, BigDecimal precision);
  ...
  @Query(value="""
                ...
                """,nativeQuery=true)
  List<SelectionZonage> selectionInterieurExterieurEpciInZoneEmploi(@Param("zoneEmploiId") List<Long> zoneEmploiId, BigDecimal precision);
}
