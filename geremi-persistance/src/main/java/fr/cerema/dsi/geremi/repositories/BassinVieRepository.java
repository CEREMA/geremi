...
/**
 * Interface de répository pour les opérations CRUD sur les objets BassinVie.
 ...
 */
public interface BassinVieRepository extends GenericRepository<BassinVie,String> {


	/**
	 * Méthode pour trouver les objets BassinVie qui se trouvent dans une boîte englobante donnée.
	 *
	 ...
	 *
	 * @return une liste d'objets BassinVie qui se trouvent dans la boîte englobante donnée.
	 */
	 @Query(value="""
                SELECT r.id as id, r.nom_bassin_vie as nom, r.code_bassin_vie as code, public.ST_REDUCEPRECISION(g.the_geom,:precision) as geomInt
                FROM ref_bassin_vie r
                INNER JOIN ref_bassin_vie_geometry g ON r.id=g.id
  ...
  @Query(value="""
                SELECT DISTINCT g.id,code_bassin_vie as code,nom_bassin_vie as nom,
                ...
                cross join ref_bassin_vie d
                join ref_bassin_vie_geometry g ON d.id=g.id
                where
                ...
                """,nativeQuery=true)
  List<SelectionZonage> selectionInterieurExterieurBassinVieInRegion(@Param("regionId") List<Long> regionId, BigDecimal precision);
  ...
  @Query(value="""
                ...
                """,nativeQuery=true)
  List<SelectionZonage> selectionInterieurExterieurBassinVieInDepartement(@Param("departementId") List<Long> departementId, BigDecimal precision);

  @Query(value="""
                SELECT DISTINCT g.id,code_bassin_vie as code,nom_bassin_vie as nom,
                public.ST_CollectionExtract(public.ST_REDUCEPRECISION(public.ST_INTERSECTION(the_geom, union_zone_emploi_geom.geom),:precision)) as geomInt,
                public.ST_CollectionExtract(public.ST_REDUCEPRECISION(public.ST_Difference(the_geom, union_zone_emploi_geom.geom),:precision)) as geomExt,
                not public.ST_IsEmpty(public.ST_Difference(the_geom, union_zone_emploi_geom.geom)) as exterieur
                FROM (
                    select public.ST_Union(the_geom) as geom from ref_zone_emploi_geometry where id in :zoneEmploiId
                ) as union_zone_emploi_geom
                cross join ref_bassin_vie d
                join ref_bassin_vie_geometry g ON d.id=g.id
                where
                  public.ST_Intersects(g.the_geom, union_zone_emploi_geom.geom)
                  and public.ST_AREA(public.ST_REDUCEPRECISION(public.ST_INTERSECTION(the_geom, union_zone_emploi_geom.geom),0.000001)) > 0
                """,nativeQuery=true)
  List<SelectionZonage> selectionInterieurExterieurBassinVieInZoneEmploi(@Param("zoneEmploiId") List<Long> zoneEmploiId, BigDecimal precision);
